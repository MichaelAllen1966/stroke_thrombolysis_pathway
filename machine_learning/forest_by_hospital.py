...
forest_results_predicted = np.zeros((7, 7))
...
for training_hospital in range(7):
    ...
    for patient_source in range(7): 
        mask =np.argmax(X[:, 0:7],axis=1) == patient_source
        X_test = X[mask,7:]
        y_pred = forest.predict(X_test)
        forest_results_predicted[training_hospital,patient_source] = (
                np.mean(y_pred))
...
forest_results_predicted.to_csv('forest_hospital_predicted.csv')
